...
const props = defineProps({
  ...
  limit: { type: Number, default: 6 } // 列表默认展示5-6人
})
...
<template>
  <div
    ...
    <div class="flex items-center gap-3 mb-2 relative z-10 pl-1">
      <div
        :class="['w-8 h-8 rounded-[10px] flex items-center justify-center shadow-sm text-white shadow-black/5', theme.bg]">
    ...
    <div class="flex-1 relative z-10">
      ...
      <div v-if="type === 'single'" class="flex flex-col h-full justify-center pb-2 pl-1">
        ...
        <div class="mt-3 flex items-center gap-2">
          <div
            :class="['px-2.5 py-1 rounded-full text-[11px] font-semibold bg-opacity-50 border border-opacity-10', theme.light, theme.text, 'border-gray-400']">
            {{ data.subtitle }}
          </div>
        </div>
      </div>

      <div v-if="type === 'list'" class="mt-1 flex flex-col">
        <div class="flex flex-col divide-y divide-gray-100/60">
          <div v-for="(item, idx) in listData" :key="idx"
            ...
            <div class="flex items-center gap-3 overflow-hidden">
              <div class="w-5 h-5 flex-shrink-0 flex items-center justify-center rounded-md text-[10px] font-bold"
                :class="idx < 3
      ...
      <div v-if="type === 'double-list'" class="mt-1 flex flex-col ">
        <!-- Top 列表 -->
        <div class="flex flex-col divide-y divide-gray-100/60">
          <div v-for="(item, idx) in doubleListData.top" :key="`top-${idx}`"
            class="flex items-center justify-between py-2 px-2 -mx-2 rounded-lg hover:bg-white/60 transition-colors duration-200 group/row cursor-default">
            <div class="flex items-center gap-3 overflow-hidden">
              <div class="w-5 h-5 flex-shrink-0 flex items-center justify-center rounded-md text-[10px] font-bold"
                :class="idx < 3
        ...
        <!-- Bottom 列表 -->
        <div class="flex flex-col divide-y divide-gray-100/60">
          <div v-for="(item, idx) in doubleListData.bottom" :key="`bottom-${idx}`"
            class="flex items-center justify-between py-2 px-2 -mx-2 rounded-lg hover:bg-white/60 transition-colors duration-200 group/row cursor-default">
            <div class="flex items-center gap-3 overflow-hidden">
              <div class="w-5 h-5 flex-shrink-0 flex items-center justify-center rounded-md text-[10px] font-bold"
                :class="(() => {
                  ...
                  if (rank < 3) {
                    ...
                    return `bg-gradient-to-br ${badgeStyle.badge} text-white ${badgeStyle.shadow}`;
                  }
